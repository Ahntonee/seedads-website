...
// POST /api/cms/upload  — keeps its own try/catch (custom "Upload failed" message)
...
// GET /api/cms/media  — keeps its own try/catch (returns { files: [] } on error, not 500)
...
// DELETE /api/cms/media/:filename  — keeps its own try/catch (returns 404 on error)
...
// ── PRICING PLANS ─────────────────────────────────────────────────────────────
router.get('/pricing-plans', asyncHandler(async (_req, res) => {
  const [rows] = await pool.query('SELECT * FROM pricing_plans WHERE published = 1 ORDER BY sort_order ASC');
  res.json({ plans: rows });
}));

router.post('/pricing-plans', requireAuth, asyncHandler(async (req, res) => {
...
router.patch('/pricing-plans/:id', requireAuth, asyncHandler(async (req, res) => {
...
router.delete('/pricing-plans/:id', requireAuth, asyncHandler(async (req, res) => {
...
router.patch('/faqs/:id', requireAuth, asyncHandler(async (req, res) => {
...
router.patch('/team/:id', requireAuth, asyncHandler(async (req, res) => {
  const { sets, vals } = buildUpdate(req.body,
...
router.patch('/portfolio/:id', requireAuth, asyncHandler(async (req, res) => {
  const { sets, vals } = buildUpdate(req.body,
...
router.patch('/case-studies/:id', requireAuth, asyncHandler(async (req, res) => {
  const { sets, vals } = buildUpdate(req.body,
    ['title','category','description','icon','image_url','stat1_value','stat1_label','stat2_value','stat2_label','stat3_value','stat3_label'],
    { description: 1000, image_url: 500 });
  if (req.body.published  !== undefined) { sets.push('published = ?');  vals.push(req.body.published  ? 1 : 0); }
...
router.delete('/case-studies/:id', requireAuth, asyncHandler(async (req, res) => {
...
router.get('/page-content', asyncHandler(async (req, res) => {
...
router.put('/page-content', requireAuth, asyncHandler(async (req, res) => {
...
router.get('/page-content/all', requireAuth, asyncHandler(async (req, res) => {
  const [rows] = await pool.query(
